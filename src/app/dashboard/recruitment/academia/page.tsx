  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/recruitment/academia/score', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ candidateId: selectedCandidate.id, submissions })
      });
      const data = await res.json();
      if (data.success) {
    ...
    <div className="min-h-screen bg-[#0D0C0A] text-cream font-sans p-8">
      ...
      <div className="flex justify-between items-center mb-12">
        ...
        <button 
          ...
          className="bg-white/5 border border-white/10 px-6 py-2.5 rounded-xl text-xs font-mono font-bold hover:bg-white/10 transition-all uppercase tracking-widest text-white/60"
        >
          Back to Recruitment
        </button>
      </div>
      ...
      {!testView ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="flex items-center space-x-4">
                  {c.academicAssessment ? (
                    <button 
                      ...
                      className="bg-gold text-ink px-5 py-2 rounded-xl text-xs font-bold hover:bg-gold/90 transition-all flex items-center"
                    >
                    ...
                    <button 
                      ...
                      className="bg-white/5 border border-white/20 text-white/60 px-5 py-2 rounded-xl text-xs font-bold hover:border-gold/50 hover:text-gold transition-all"
                    >
                      {isGenerating ? <Loader2 className="animate-spin w-4 h-4" /> : 'Generate Question Paper'}
                    </button>
                  )}
                </div>
              </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-gold/5 border border-gold/10 rounded-3xl p-8 relative overflow-hidden">
              ...
              <h4 className="text-xs font-mono font-bold text-gold uppercase tracking-widest mb-6">Academia Intelligence</h4>
              <p className="text-white/60 text-sm leading-relaxed mb-6">
                The academic round evaluates candidates on theoretical foundations. AI models generate role-specific questions across:
              </p>
              <ul className="space-y-4 text-sm text-white/80">
                <li className="flex items-center gap-3"><CheckCircle className="w-4 h-4 text-gold" /> Data Structures & Algorithms</li>
                <li className="flex items-center gap-3"><CheckCircle className="w-4 h-4 text-gold" /> System Architecture</li>
                <li className="flex items-center gap-3"><CheckCircle className="w-4 h-4 text-gold" /> Domain Specific Patterns</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <AnimatePresence>
          <motion.div 
            ...
            className="max-w-4xl mx-auto bg-white/5 border border-white/10 rounded-3xl p-10 shadow-2xl relative overflow-hidden"
          >
            ...
            <div className="mb-10 pb-10 border-b border-white/5">
              ...
              <p className="text-gold font-mono text-sm tracking-widest uppercase italic">{selectedCandidate?.role} Technical Assessment</p>
            </div>
                ...
                <div key={q.id} className="space-y-4 bg-white/5 p-8 rounded-2xl border border-white/5">
                  ...
                  {q.type === 'mcq' ? (
                    ...
                    <textarea 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl p-5 text-sm text-white/80 placeholder:text-white/20 focus:outline-none focus:border-gold/40 h-32 mt-4"
                    />
                  )}
                </div>
              ))}
            ...
            <div className="mt-12 flex justify-end gap-4">
              <button 
                ...
                className="px-8 py-3 rounded-xl border border-white/10 text-white/40 hover:text-white/70 transition-all font-bold text-sm uppercase tracking-widest"
              >
                Cancel
              </button>
              <button 
